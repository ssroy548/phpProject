...
include("includes/connection.php");
	...
	if(isset($_POST['sign_up'])){
		...
		if($check == 1){
			echo "<script>alert('Email already exist, Please try again using another email')</
			script>";
		...
		$rand = rand(1,3);
			...
			$insert = "insert into users(f_name,l_name,user_name,describe_user,Relationship,user_pass,
				user_email,user_country,user_gender,user_birthday,user_image,user_cover,user_reg_date,status,posts,recovery_account) 
				values ('$first_name','$last_name','$username','Hello guys.This is my default status!','...','$pass','$email','$country','$gender','$birthday','$profile_pic','cover.jpg',NOW(),'$status','$posts','iwanttoputadinginthe
				universe.')";
